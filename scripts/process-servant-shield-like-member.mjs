/**
 * Exact member-shield pipeline (43219725 → member-shield.png v13)
 * with isBlue → isGreen for the servant field color.
 */
...
const out = process.argv[3] ?? "public/shields/servant-shield.png";
if (!src) {
  console.error("Usage: node scripts/process-servant-shield-like-member.mjs <src> [out]");
  process.exit(1);
}
...
// ── Step 1: black-key cutout (member line 965) ──
...
// ── Step 2: remove disconnected gray Gemini star (member line 970) ──
...
function isGrayStar(r, g, b) {
  const avg = (r + g + b) / 3;
  const sat = Math.max(r, g, b) - Math.min(r, g, b);
  return sat < 28 && avg > 45 && avg < 210;
}

function isShieldPixelStar(r, g, b) {
  if (g > r + 18 && g > 70) return true; // green field (mirror: b > r + 18 && b > 70)
  if (r > 85 && g > 40 && b < 110 && r >= g - 12) return true; // gold
  if (g > r + 10 && g > 60 && b < g) return true; // greenish gold edge rare
  return false;
}
...
for (let sy = Math.floor(h * 0.88); sy < h; sy++) {
  for (let sx = Math.floor(w * 0.78); sx < w; sx++) {
    const start = sy * w + sx;
    const di = start * 4;
    ...
    const g = px[di + 1];
    const b = px[di + 2];
    if (!isGrayStar(r, g, b) || isShieldPixelStar(r, g, b)) continue;
    ...
    while (stack.length) {
      ...
      const cg = px[ci + 1];
      const cb = px[ci + 2];
      if (isShieldPixelStar(cr, cg, cb)) touchesShield = true;
      ...
      for (const [dx, dy] of [
        ...
        const ng = px[ni * 4 + 1];
        const nb = px[ni * 4 + 2];
        if (!isGrayStar(nr, ng, nb)) continue;
        visited[ni] = 1;
        stack.push(ni);
      }
    }
...
// ── Step 3: remove below shield + bottom-right gray (member line 972) ──
...
function isShieldPixel(r, g, b) {
  if (g > r + 15 && g > 65) return true; // green field (mirror: b > r + 15 && b > 65)
  if (r > 85 && g > 40 && b < 115 && r >= g - 15) return true;
  return false;
}

function isGrayWatermark(r, g, b) {
  const sat = Math.max(r, g, b) - Math.min(r, g, b);
  const avg = (r + g + b) / 3;
...
for (let y = 0; y < h; y++) {
  for (let x = 0; x < w; x++) {
    ...
    const g = px[di + 1];
    const b = px[di + 2];
    if (isShieldPixel(r, g, b)) maxShieldY = Math.max(maxShieldY, y);
  }
}
...
for (let y = 0; y < h; y++) {
  for (let x = 0; x < w; x++) {
    ...
    const bottomRightGray = y > h * 0.88 && x > w * 0.72 && isGrayWatermark(r, g, b);
...
// ── Step 4: remove gray sparkle + inpaint field holes (member line 984) ──
buf = await sharp(outPath).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
w = buf.info.width;
h = buf.info.height;
px = Buffer.from(buf.data);
...
function isGold(r, g, b) {
  return r > 85 && g > 40 && b < 115 && r >= g - 15;
}
function isGreen(r, g, b) {
  return g > r + 12 && g > 65; // mirror: b > r + 12 && b > 65
}
function isGray(r, g, b) {
  const sat = Math.max(r, g, b) - Math.min(r, g, b);
  const avg = (r + g + b) / 3;
  return sat < 32 && avg > 40 && avg < 220;
}
...
for (let y = 0; y < h; y++) {
  for (let x = 0; x < w; x++) {
    ...
    const g = px[di + 1];
    const b = px[di + 2];
    if (isGold(r, g, b) || isGreen(r, g, b)) continue;
...
for (let pass = 0; pass < 2; pass++) {
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      if (greens.length >= 3) {
        px[di] = Math.round(greens.reduce((s, p) => s + p.r, 0) / greens.length);
        px[di + 1] = Math.round(greens.reduce((s, p) => s + p.g, 0) / greens.length);
        px[di + 2] = Math.round(greens.reduce((s, p) => s + p.b, 0) / greens.length);
        px[di + 3] = 255;
      }
    }
  }

/**
 * Priest shield — black cutout only (member step 1 + safe step 2).
 * Keeps white field + gold art; does NOT strip interior gray/shadows.
 */
...
const out = process.argv[3] ?? "public/shields/priest-shield.png";
if (!src) {
  console.error("Usage: node scripts/process-priest-shield-like-member.mjs <src> [out]");
  process.exit(1);
}
...
function isWhiteField(r, g, b) {
  return r > 165 && g > 165 && b > 165;
}

function isShieldPixel(r, g, b) {
  if (isGold(r, g, b)) return true;
  if (isWhiteField(r, g, b)) return true;
  if (r > 120 && g > 110 && b > 95 && r - b < 90) return true;
  return false;
}
...
// ── Step 1: black-key cutout (member pipeline) ──

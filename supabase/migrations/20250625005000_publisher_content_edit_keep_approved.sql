-- Keep approved content approved on metadata-only edits; re-review only when media changes.
...
begin
  ...
  if v_item.is_trusted then
    v_status := 'approved';
  elsif v_item.status = 'approved' then
    if p_media_url is not null and nullif(trim(p_media_url), '') is distinct from v_item.media_url then
    ...
    v_status := 'pending_review';
    v_needs_review := v_item.status is distinct from 'pending_review';
  end if;
  ...
  if v_status = 'pending_review' and v_needs_review then
  ...
  return json_build_object(
    ...
    'reReview', v_needs_review and v_status = 'pending_review'
  );
end;
$$;

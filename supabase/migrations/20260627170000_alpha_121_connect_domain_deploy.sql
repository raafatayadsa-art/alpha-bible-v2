-- DOMAIN-04 Alpha Connect — deploy presence + nearby/contacts (ALPHA-121 P1)
-- Supersedes production gap from 20250618190000 (no legacy profiles table; uses user_profiles)

-- ---------------------------------------------------------------------------
-- Presence (ALPHA-PRESENCE-STATUS-002)
-- ---------------------------------------------------------------------------
...
comment on table public.alpha_user_presence is
  'DOMAIN-04 Alpha Connect: user presence status (available/busy/hidden)';

-- ---------------------------------------------------------------------------
-- Nearby discovery + connection graph
-- ---------------------------------------------------------------------------
...
drop policy if exists alpha_user_discovery_prefs_select_self on public.alpha_user_discovery_prefs;
create policy alpha_user_discovery_prefs_select_self
  on public.alpha_user_discovery_prefs
  for select
  to authenticated
  using (auth.uid() = user_id);

drop policy if exists alpha_user_discovery_prefs_write_self on public.alpha_user_discovery_prefs;
create policy alpha_user_discovery_prefs_write_self
  on public.alpha_user_discovery_prefs
  for all
  to authenticated
  using (auth.uid() = user_id)
...
drop policy if exists alpha_connect_connection_requests_select_parties on public.alpha_connect_connection_requests;
create policy alpha_connect_connection_requests_select_parties
  on public.alpha_connect_connection_requests
  for select
  to authenticated
  using (auth.uid() in (from_user_id, to_user_id));

drop policy if exists alpha_connect_contacts_select_self on public.alpha_connect_contacts;
create policy alpha_connect_contacts_select_self
  on public.alpha_connect_contacts
  for select
  to authenticated
  using (auth.uid() = user_id);

comment on table public.alpha_user_discovery_prefs is
  'DOMAIN-04 Alpha Connect: opt-in nearby discovery preferences and location heartbeat';
comment on table public.alpha_connect_connection_requests is
  'DOMAIN-04 Alpha Connect: member connection requests (pending/accepted/rejected)';
comment on table public.alpha_connect_contacts is
  'DOMAIN-04 Alpha Connect: bilateral contacts after accepted request';
...
begin
  ...
  select
    ...
    coalesce(c.church_name, 'كنيسة Alpha') as church_name,
    coalesce(cm.role_label, cm.role, 'member') as role_label,
    (
      6371000 * acos(
  ...
  left join public.user_profiles p on p.user_id = dp.user_id
  left join public.church_memberships cm on cm.user_id = dp.user_id and cm.status = 'active'
...
comment on function public.alpha_nearby_members(double precision, double precision, integer) is
  'DOMAIN-04 Alpha Connect: discover nearby discoverable church members (ALPHA-099)';

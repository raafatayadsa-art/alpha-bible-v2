-- Seed platform_modules + batch save RPC (security definer).
-- Fixes empty-table save failures when direct UPDATE/INSERT is blocked for anon.

-- 1) Ensure all known module keys exist
...
-- 2) Batch save module enabled flags
...
-- 3) Upsert-safe single toggle (keep in sync)

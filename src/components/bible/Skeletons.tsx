...
function Shimmer({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-[color-mix(in_srgb,var(--alpha-border)_60%,transparent)] rounded-xl",
  ...
  return (
    <div className="grid grid-cols-3 gap-2.5" aria-hidden>
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="rounded-[var(--alpha-radius-dock-tab)] bg-[color-mix(in_srgb,var(--alpha-bg-elevated)_70%,transparent)] border border-alpha p-2.5"
        >
...
export function ChapterGridSkeleton({ count = 20 }: { count?: number }) {
  return (
    <ul className="grid grid-cols-5 gap-2.5" aria-hidden>
      {Array.from({ length: count }).map((_, i) => (
        <li key={i}>
          <Shimmer className="aspect-square rounded-[var(--alpha-radius-dock-tab)]" />
        </li>
      ))}
    </ul>
  );
  ...
  return (
    <div className="mt-5 space-y-2.5" aria-hidden>
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="rounded-[var(--alpha-radius-dock-tab)] bg-white/55 border border-alpha/70 px-3.5 py-3"
        >

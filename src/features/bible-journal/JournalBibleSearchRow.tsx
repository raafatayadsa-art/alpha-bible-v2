...
/** Bible 2 / Alpha expandable search — navigates to scripture results. */
export function JournalBibleSearchRow() {
    ...
    <div dir="rtl" className="mt-4">
      ...
      {searchExpanded && trimmedQuery ? (
        <div className="alpha-connect-theme alpha-connect-theme--classic mt-2.5 space-y-2">
          {results.length === 0 ? (
            <p className="py-4 text-center text-[12px] font-medium" style={{ color: JOURNAL_VAULT.textMuted }}>
              لا توجد نتائج
            </p>
          ) : (
            results.map((result) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {result.subtitle ? (
                    <div className="mt-0.5 line-clamp-2 text-[11px]" style={{ color: "#6a543a" }}>
                      {result.subtitle}
                    </div>
                  ) : null}
                </div>
                <ChevronLeft className="h-4 w-4 shrink-0" style={{ color: JOURNAL_VAULT.gold }} />
              </button>
            ))
          )}
        </div>
